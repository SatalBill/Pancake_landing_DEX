...
>('ethpredictions/fetchRoundBet', async ({ account, roundId }) => {
...
>('ethpredictions/fetchCurrentBets', async ({ account, roundIds }) => {
  const betResponses = await getBetHistory({
    user: account.toLowerCase(),
    round_in: roundIds,
  })
