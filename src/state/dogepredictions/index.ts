...
>('dogepredictions/fetchRoundBet', async ({ account, roundId }) => {
...
>('dogepredictions/fetchCurrentBets', async ({ account, roundIds }) => {
  const betResponses = await getBetHistory({
    user: account.toLowerCase(),
    round_in: roundIds,
  })
