...
>('bnbpredictions/fetchRoundBet', async ({ account, roundId }) => {
...
>('bnbpredictions/fetchCurrentBets', async ({ account, roundIds }) => {
  const betResponses = await getBetHistory({
    user: account.toLowerCase(),
    round_in: roundIds,
  })
